...
- **Credit-card pockets.** Mark a pocket as `kind=credit` with statement and due days; the Pockets list grows a separate **Cards** section showing outstanding amount, current-cycle spend, and the next bill with days-to-due. Pay it off via a pre-filled transfer from any cash pocket. The Dashboard's headline splits into **Cash / Owed / Net** so you can see at a glance whether you'd still be positive after paying every card in full.
- **Installment / cicilan plans.** When entering a credit-card expense, pick an installments duration (3/6/12/24 months); the app materialises one Transaction per installment so cycle math, the bill due figure, and the per-month statement all stay simple. The card detail page also shows an **Outstanding / This cycle / Bill due / Committed** four-figure layout (Committed = sum of future installments) plus an Active plans summary. Future-dated installment rows are hidden by default in the Transactions list and Dashboard latest list — toggle "Show planned" on the Transactions filter to see them.
